  ...
  <div>
    <div>
      ...
      <el-table :data="users" border class="table" header-cell-class-name="table-header">
        ...
        <el-table-column prop="username" label="用户名" style="align-items: center;"></el-table-column>
        <el-table-column prop="phoneNumber" label="手机号" style="align-items: center;"></el-table-column>
        <el-table-column prop="email" label="邮箱" style="align-items: center;"></el-table-column>
        <el-table-column label="赞赏" width="100" style="align-items: center;">
          <template slot-scope="scope">
            <el-input size="medium" maxlength="30" v-model="scope.row.admire"
                      @blur="changeUserAdmire(scope.row)"></el-input>
          </template>
        </el-table-column>
        <el-table-column label="用户状态" style="align-items: center;">
          <template slot-scope="scope">
            <el-tag :type="scope.row.userStatus === false ? 'danger' : 'success'"
                    disable-transitions>
              {{scope.row.userStatus === false ? '禁用' : '启用'}}
            </el-tag>
            <el-switch v-if="scope.row.id !== $store.state.currentAdmin.id" @click="changeUserStatus(scope.row)" v-model="scope.row.userStatus"></el-switch>
          </template>
        </el-table-column>
        <el-table-column label="头像" style="align-items: center;">
          <template slot-scope="scope">
            <el-image lazy class="table-td-thumb" :src="scope.row.avatar" fit="cover"></el-image>
          </template>
        </el-table-column>
        <el-table-column label="性别" style="align-items: center;">
          <template slot-scope="scope">
            <el-tag type="success"
        ...
        <el-table-column prop="introduction" label="简介" style="align-items: center;"></el-table-column>
        <el-table-column label="用户类型" width="100" style="align-items: center;">
          <template slot-scope="scope">
            <el-tag type="success"
        ...
        <el-table-column prop="createTime" label="注册时间" style="align-items: center;"></el-table-column>
      </el-table>
